...
def check_and_reset(chat_key):
    ...
    if room_data["last_reset"] != today:
        ...
        for user in yesterday_users.values():
            if user["count"] < 30:
                user["total_under_count"] = (
                    user.get("total_under_count", 0) + 1
                )
        ...
        if under_active:
            ...
            try:
                line_bot_api.push_message(
                    chat_key, TextSendMessage(text=warning_msg)
                )
            except Exception:
                pass
...
def handle_join(event):
    try:
        ...
        line_bot_api.reply_message(
            event.reply_token, TextSendMessage(text=welcome_msg)
        )
    except Exception:
        pass
    return "OK"
...
def handle_leave(event):
    try:
        chat_key = getattr(event.source, "group_id", None) or getattr(
            event.source, "room_id", None
        )
        if chat_key:
            line_bot_api.push_message(
                chat_key,
                TextSendMessage(text="#ㄴㄱ (멤버가 퇴장하셨습니다 😢)"),
            )
    except Exception:
        pass
    return "OK"
...
def handle_message(event):
    ...
    # 2. 순위 확인
        ...
        sorted_users = sorted(
            user_data.values(), key=lambda x: x["count"], reverse=True
        )
    ...
    # 5. 수동 초기화
        ...
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text="🔄 현재 방의 데이터가 초기화되었습니다."),
        )
